# -*- coding: utf-8 -*-
"""
2026-08-21 盘后定稿引擎（复刻 build_0820.py；周六补跑 08-21 缺口）
数据源：现有 import_final.json(156) 复用内嵌日K + qt.gtimg.cn 叠加 08-21 收盘棒；
        新筛候选经 web.ifzq.gtimg.cn(qfq,800) 拉新鲜日K。评分全自 K线推导。
候选池：现有重评 + 08-21 tool_filter(main_inflow>=1.5亿,剔除涨停追高>9.5%) + high_dividend(>=4%)。
输出：选股结果/2026-08-21.md + 选股结果/import_final.json（合并去重，池只增不减）。
"""
...
NEW_SCREEN={
 # 08-21 主力净流入>=1.5亿（剔除涨停/追高 ChangePCT>9.5%）
 '300308':'中际旭创','300502':'新易盛','600183':'生益科技','002241':'歌尔股份','601899':'紫金矿业',
 '002463':'沪电股份','002384':'东山精密','300285':'国瓷材料','001309':'德明利','601138':'工业富联',
 '000426':'兴业银锡','603986':'兆易创新','000657':'中钨高新','600547':'山东黄金','000725':'京东方Ａ',
 '002484':'江海股份','600498':'烽火通信','002851':'麦格米特','002436':'兴森科技','002281':'光迅科技',
 '300750':'宁德时代','300390':'天华新能','600489':'中金黄金','600549':'厦门钨业','000630':'铜陵有色',
 '301205':'联特科技','603993':'洛阳钼业','600150':'中国船舶','688300':'联瑞新材','600869':'远东股份',
 '300476':'胜宏科技','601869':'长飞光纤','688525':'佰维存储','601919':'中远海控','603228':'景旺电子',
 '002156':'通富微电','600500':'中化国际','603061':'金海通','603799':'华友钴业','002460':'赣锋锂业',
 '000703':'恒逸石化','000737':'北方铜业','001203':'大中矿业','600378':'昊华科技','300274':'阳光电源',
 # 08-21 高股息>=4%（防御向补充；已在池者由脚本去重）
 '002763':'汇洁股份','603165':'荣晟环保','600329':'达仁堂','603508':'思维列控','603569':'长久物流',
 '000915':'华特达因','603167':'渤海轮渡','300406':'九强生物','300533':'冰川网络','002004':'华邦健康',
 '002572':'索菲亚','000913':'钱江摩托','600219':'南山铝业','603551':'奥普科技','600866':'星湖科技',
 '601216':'君正集团','600983':'惠而浦','300994':'久祺股份','600066':'宇通客车','601717':'中创智领',
 '300770':'新媒股份','002271':'东方雨虹','002612':'朗姿股份','688399':'硕世生物','600861':'北京人力',
 '002616':'长青集团','600153':'建发股份','001323':'慕思股份','301207':'华兰疫苗','603833':'欧派家居',
 '002007':'华兰生物','605266':'健之佳','002867':'周大生','000651':'格力电器','000858':'五 粮 液',
 '600566':'济川药业','600398':'海澜之家','002027':'分众传媒','600742':'富维股份','601083':'锦江航运'}
...
# ----- 构建 kl_map：现有复用内嵌日K + 叠加当日棒；新候选拉新鲜 -----
kl_map={}
for c in existing_codes:
    emb=old_map[c].get('kline',{}).get('day')
    if emb and len(emb)>=30:
        kl_map[c]=[list(b) for b in emb]   # 复制，避免改到旧对象
    else:
        kl_map[c]=None  # 缺失则拉新鲜

def worker(code):
    return code, pull(code)
with ThreadPoolExecutor(max_workers=8) as ex:
    futs=[ex.submit(worker,c) for c in new_codes]
...
print(f'K线就绪 {ok}/{len(all_codes)} (现有复用+叠加 {len(existing_codes)-sum(1 for c in existing_codes if kl_map[c] is None)} 只, 新拉 {len(new_codes)})')
...
def evaluate(code, name, is_new):
    ...
    if len(day)<MIN_BARS:
        SKIP_SHORT.append((name, len(day), day[0][0]))
        return None
    m=metrics(day)
    m['ret20']=ret20(day)
    board=board_of(code)
...
if SKIP_SHORT:
    print(f'次新剔除(日K<{MIN_BARS}根): ' + '、'.join(f'{n}({l}根,首棒{d})' for n,l,d in SKIP_SHORT))

# ----- 写 import_final.json -----
def setcode_of(code):
    return '1' if code[0] in '69' else '0'

new_final=[]
for r in rows:
    if r['pass_'] and r['is_new']:
        new_final.append(dict(name=r['name'], code=r['code'], board=r['board'], setcode=setcode_of(r['code']),
            ma20=r['m']['ma20'], priceMa=r['m']['priceMa'], ma60=r['m']['ma60'],
            rsi=r['m']['rsi'], vol=r['m']['vol'], struct=r['m']['struct'], sector=r['sector'],
            atr=r['m']['atr'], score=r['total'], win=r['strength'], entry=r['entry'], category='final', date=DATE,
...
for it in old_items:
    ...
    if rr['pass_']:
        new_it.update(dict(date=DATE, entry=rr['entry'], stopPrice=rr['stop'], targetPrice=rr['target'],
                           score=rr['total'], win=rr['strength']))
...
# ----- 大盘实况（qt.gtimg.cn 三指数） -----
def idx_pct(sym, tries=3):
    last=None
    for _ in range(tries):
        try:
            t=get('https://qt.gtimg.cn/q='+sym, enc='gbk')
            f=t.split('~')
            price=float(f[3]); prev=float(f[4])
            return f[1], round((price-prev)/prev*100,2)
        except Exception as e:
            last=e; time.sleep(1.0)
    return sym,'?'
idxs=[idx_pct('sh000001'), idx_pct('sz399001'), idx_pct('sz399006')]
def _fmt(n,p):
    return f"{n}{p:+.2f}%" if isinstance(p,(int,float)) else f"{n}{p}"
idx_line='；'.join(_fmt(n,p) for n,p in idxs)
drop2=any(isinstance(p,(int,float)) and p<=-2 for _,p in idxs)
print('大盘:', idx_line, '| Step-8触发=', drop2)
...
for it in old_items:
    ...
    sp=it.get('stopPrice')
    k=kl_map.get(c)
    if not sp or not k: continue
    if it.get('date')==DATE: continue   # 当日新签不算破位
    close=k[-1][2]
    if close < sp:
        broken.append((it['name'], c, round(close,2), sp, round((close-sp)/sp*100,2)))
broken.sort(key=lambda x:x[4])
print(f'存量破位 {len(broken)} 只')
...
lines.append('')
lines.append(f'- **数据日**：{DATE}（周六补跑 08-21 周五收盘）')
...
for r in fails[:25]:
    ...
    if any(str(c).startswith('主板回踩拒') for c in r['caveats']):
        why.append('主板回踩带拒（须价在MA20上方且收盘≤MA20×1.05）')
    why_s='、'.join(why) or '技术面未达标'
    lines.append(f"- {r['name']}（{r['board']}）：{why_s} 〔分{r['total']}/胜率{r['strength']:.1f}%〕")
lines.append('')
lines.append('## 四、大盘实况与资金主线')
lines.append('')
lines.append(f'- 三大指数（{DATE} 收盘）：{idx_line}。')
if drop2:
    lines.append('- ⚠️ **系统性回调警示（Step-8 触发）**：三大指数任一跌超2%，建议收缩或暂停新增开仓。')
else:
    lines.append('- 未触发 Step-8 跌超2% 收缩警示；资金主线与行业配置详见 fundflow.json（资金动态面板）。')
lines.append('')
lines.append('## 五、存量持仓破位警示（收盘跌破历史签发止损价）')
lines.append('')
if broken:
    lines.append(f'- 共 {len(broken)} 只收盘跌破历史签发止损价，按纪律处理（展示前25，按偏离幅度排序）：')
    for nm,c,cl,sp,dev in broken[:25]:
        lines.append(f"  - {nm}：现价 {cl} < 止损 {sp}（偏离 {dev:+.2f}%）")
else:
    lines.append('- 无存量破位。')
lines.append('')
lines.append('## 六、风险提示')
