# -*- coding: utf-8 -*-
"""
2026-08-19 盘后定稿引擎（复刻 build_final.py 6维评分 + 任务 step3 ATR 规则 + 用户风控基线）
数据源：腾讯公开端点 web.ifzq.gtimg.cn 拉日K(qfq,800,末棒滞后1日→qt.gtimg.cn 叠加当日收盘)，评分全自 K线推导。
候选池：现有 import_final.json(147) + 08-18 tool_filter(main_inflow>=1.5亿, 剔除涨停追高>9.5%) + high_dividend(>=4%)。
输出：选股结果/2026-08-19.md + 选股结果/import_final.json（合并去重，池只增不减）。
"""
