# -*- coding: utf-8 -*-
"""
2026-08-27 盘后定稿引擎（派生自 build_0826.py；周四 2026-08-27 收盘全量重跑）
数据源：现有 import_final.json(183) 复用内嵌日K + qt.gtimg.cn 叠加 08-27 收盘棒；
        新筛候选经 web.ifzq.gtimg.cn(qfq,800) 拉新鲜日K。评分全自 K线推导。
候选池：现有重评 + 08-27 tool_filter(main_inflow + high_dividend，剔除涨停追高>9.5%)。
输出：选股结果/2026-08-27.md + 选股结果/import_final.json（合并去重，池只增不减）。
"""
...
NEW_SCREEN = (json.load(open(os.path.join(RES, "screen_0827.json"), encoding='utf-8'))
              if os.path.exists(os.path.join(RES, "screen_0827.json")) else {})
# 08-27 新筛候选（westock tool_filter: main_inflow>=1亿 + high_dividend>=4%，已剔除涨停/追高>9.5%）
...
lines.append('')
lines.append(f'- **数据日**：{DATE}（周四收盘盘后定稿）')
