...
const STRESS_BEARISH = process.env.STRESS_BEARISH === '1'; // 压力测试：放宽③趋势过滤，强制在空头日也出信号
const OUT = STRESS_BEARISH ? 'D:/WorkBuddy/选股结果/backtest_bearish_stress.json' : 'D:/WorkBuddy/选股结果/backtest_winrate.json';
...
function genSignals(stock) {
  ...
  for (let i = 0; i < bars.length - 1; i++) {
    ...
    if (!STRESS_BEARISH && !f.trendOk) { preStats.skipTrend++; continue; } // 压力测试下不要求趋势过滤
...
try {
  ...
  // 注意：index_sh.json 的 bars 值是字符串，必须转数字，否则 MA20 会变成字符串拼接、cl<ma 退化为字典序比较（大盘过滤静默失效）
  const ib = (idx.bars || []).map(b => [b[0], parseFloat(b[1]), parseFloat(b[2]), parseFloat(b[3]), parseFloat(b[4]), parseFloat(b[5])]);
...
// 大盘市况分层：空头(上证<MA20 信号日) vs 正常 —— 用于评估大盘硬过滤的价值
function regimeWinRate(list) {
  const t = list.length, w = list.filter(x => x.outcome === 'win').length;
  return { trades: t, wins: w, losses: t - w, winRate: t ? +(w / t).toFixed(4) : 0 };
}
const byMarketRegime = {
  bearish: regimeWinRate(idxFiltered),   // 空头日信号（被大盘硬过滤剔除的那批）
  normal:  regimeWinRate(afterIndex)     // 正常日信号（未触发大盘过滤）
};
// 若完全不做大盘过滤，合并胜率
const combinedNoFilter = regimeWinRate(candidates);
...
const result = {
  ...
  byBoard,
  byWinTier,                                   // ① 分层校验
  byMarketRegime,                              // 大盘市况分层：空头 vs 正常
  combinedNoFilter,                            // 不做大盘过滤的整体胜率（对照）
  idxFilteredDetail: idxFiltered,              // 保留空头日交易明细，供后续分析
  calibration: {                                // ① 标定
...
console.log('\n=== 大盘市况分层胜率 ===');
console.log('空头日(上证<MA20):', (byMarketRegime.bearish.winRate * 100).toFixed(1) + `%(${byMarketRegime.bearish.wins}/${byMarketRegime.bearish.trades})`,
            '| 正常日:', (byMarketRegime.normal.winRate * 100).toFixed(1) + `%(${byMarketRegime.normal.wins}/${byMarketRegime.normal.trades})`);
console.log('不做大盘过滤整体 :', (combinedNoFilter.winRate * 100).toFixed(1) + `%(${combinedNoFilter.wins}/${combinedNoFilter.trades})`,
            '| 大盘过滤剔除空头交易', idxFiltered.length, '笔');
