# -*- coding: utf-8 -*-
"""
买入信号生成 (2026-09-03 09:30 自动化)
与回测 backtest_winrate.js 同源：v1.0.4 全市场趋势+量能+缺口过滤 / v1.0.5 拉长历史 / v1.0.6 大盘硬过滤+同日上限3

⚠ 本次特殊情形：2026-09-02 的 15:40 盘后定稿未执行，import_final.json 的 kline.day 末根仍为 2026-09-01。
   为保持「信号日 = 前一交易日」与回测同源，本脚本重新拉取日K，取 2026-09-02 完成棒作为基准日/信号日；
...
for it in top:
    ...
    if len(day) < 21 or day[-1][0] != BASELINE_DATE:
        rec.update({'baseline': (day[-1][2] if day else None), 'open': None,
                    ...
                    'reason': '基准日K缺失或不对齐(末根=%s)' % (day[-1][0] if day else 'NA')})
