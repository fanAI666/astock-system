...
TODAY = '2026-09-18'
WIN_START = '2026-08-20'   # 近窗起点，足够覆盖 09-16 之后新增棒(09-17/09-18)
...
def fetch_recent_tx(code):
    """取近窗日K(腾讯 qfq 端点)，返回 [date,open,close,high,low,volume] 老->新；None 表示失败/空。"""
...
# ---- 数据源：本地日K库优先（baostock 前复权，与腾讯 qfq 近端偏差 0.000%），腾讯端点兜底 ----
# 本地库价值：每日 1270 次逐只 HTTP -> 0 次；库缺失/滞后时自动降级，行为不中断。
_LOCAL = None
try:
    import klines_local as _LOCAL
    HAS_LOCAL = _LOCAL.db_available()
except Exception as _e:
    HAS_LOCAL = False
    print('  [!] 本地库模块不可用，全部走腾讯端点:', _e)

LOCAL_STATS = {'hit': 0, 'miss': 0, 'stale': 0}

def fetch_recent_local(code):
    """从本地库取近窗日K；库缺失/该股无数据/数据滞后 -> None（触发腾讯兜底）"""
    if not HAS_LOCAL:
        return None
    try:
        bars = _LOCAL.recent_bars(code, WIN_START, TODAY, WIN_COUNT)
    except Exception:
        return None
    if not bars:
        return None
    # 滞后守卫：本地库末根应 >= TODAY 前 5 个自然日（覆盖周末），否则视为未同步
    try:
        lag = (datetime.date.fromisoformat(TODAY) - datetime.date.fromisoformat(bars[-1][0])).days
    except Exception:
        lag = 99
    if lag > 5:
        LOCAL_STATS['stale'] += 1
...
def fetch_recent(code):
    """统一入口：本地库优先，腾讯兜底"""
    bars = fetch_recent_local(code)
    if bars:
        LOCAL_STATS['hit'] += 1
        return bars
    LOCAL_STATS['miss'] += 1
    return fetch_recent_tx(code)
...
lines.append('> 数据源：历史日K取自本地库（baostock 前复权，与腾讯 qfq 同口径，需人工复核时以当日行情为准）；当日收盘棒取自腾讯 qt.gtimg.cn 权威源；日K已叠加当日 %s 收盘棒；评分引擎：6 维加权模型' % TODAY)
lines.append('')
lines.append('## 一、市场概览')
lines.append('')
...
print('日K数据源: 本地库命中 %d / 腾讯兜底 %d (其中滞后降级 %d)' % (
    LOCAL_STATS['hit'], LOCAL_STATS['miss'], LOCAL_STATS['stale']))
if HAS_LOCAL:
    try:
        _cov = _LOCAL.coverage()
        print('本地库: %s 行 / %s 只 / %s ~ %s' % (
            format(_cov.get('rows', 0), ','), _cov.get('symbols'), _cov.get('min'), _cov.get('max')))
    except Exception:
        pass
