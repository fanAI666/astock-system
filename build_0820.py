# -*- coding: utf-8 -*-
"""
2026-08-20 盘后定稿引擎（复刻 build_final.py 6维评分 + 任务 step3 ATR 规则 + 用户风控基线）
数据源：腾讯公开端点 web.ifzq.gtimg.cn 拉日K(qfq,800,末棒滞后1日→qt.gtimg.cn 叠加当日收盘)，评分全自 K线推导。
候选池：现有 import_final.json(151) + 08-20 tool_filter(main_inflow>=1.5亿, 剔除涨停追高>9.5% 与次新 N绿控) + high_dividend(>=4%)。
输出：选股结果/2026-08-20.md + 选股结果/import_final.json（合并去重，池只增不减）。
"""
...
NEW_SCREEN={
 # 08-20 主力净流入 >=1.5亿（剔除涨停/追高 ChangePCT>9.5% 与次新 N绿控传动 sz301655）
 '601899':'紫金矿业','002821':'凯莱英','603259':'药明康德','688313':'仕佳光子','688766':'普冉股份',
 '600487':'亨通光电','000426':'兴业银锡','000603':'盛达资源','300433':'蓝思科技','688167':'炬光科技',
 '600988':'赤峰黄金','002594':'比亚迪','300476':'胜宏科技','002353':'杰瑞股份','600176':'中国巨石',
 '688037':'芯源微','300418':'昆仑万维','300548':'长芯博创','688361':'中科飞测','300139':'晓程纸',
 '600489':'中金黄金','601318':'中国平安','000975':'山金国际','603127':'昭衍新药','600887':'伊利股份',
 '688120':'华海清科','301171':'易点天下','688025':'杰普特','600869':'远东股份','002714':'牧原股份',
 '603083':'剑桥科技','688235':'百济神州','000506':'招金黄金','002716':'湖南白银','600036':'招商银行',
 '600415':'小商品城','600547':'山东黄金','600584':'长电科技',
 # 08-20 高股息 >=4%（防御向补充；已在池者由脚本去重；剔除华兰疫苗+15.78%追高）
 '002763':'汇洁股份','603165':'荣晟环保','600329':'达仁堂','603508':'思维列控','603569':'长久物流',
 '603167':'渤海轮渡','300533':'冰川网络','300406':'九强生物','002004':'华邦健康','002572':'索菲亚',
 '000913':'钱江摩托','000915':'华特达因','600219':'南山铝业','603551':'奥普科技','601216':'君正集团',
 '600983':'惠而浦','300994':'久祺股份','600866':'星湖科技','601717':'中创智领','600066':'宇通客车',
 '300770':'新媒股份','002271':'东方雨虹','002616':'长青集团','600861':'北京人力','002612':'朗姿股份',
 '688399':'硕世生物','600153':'建发股份','603833':'欧派家居','001323':'慕思股份','002007':'华兰生物',
 '002867':'周大生','000651':'格力电器','605266':'健之佳','000858':'五 粮 液','601083':'锦江航运',
 '600566':'济川药业','600742':'富维股份','002027':'分众传媒','600398':'海澜之家'}
...
for it in old_items:
    ...
    new_it.update(dict(ma20=rr['m']['ma20'], priceMa=rr['m']['priceMa'], ma60=rr['m']['ma60'],
        ...
        atr=rr['m']['atr']))
    if rr['pass_']:
        # 重新签发当日信号（补写 score/win，修复 score 与 MD 脱钩导致站点不渲染达标）
