# -*- coding: utf-8 -*-
"""
每日收盘选股定稿 (2026-09-08, 15:40)
- 刷新 import_final.json 中全部候选股日K线(≈800根)
- 叠加当日(09-08)收盘棒(qt.gtimg.cn 为权威源，因 fqkline 端点末根不一致：部分含09-08、部分仍09-07)
- 基于含当日棒的日K重算 6 维评分 + 胜率估算
- 仅保留达标(胜率>=70%)标的 -> 写入 选股结果/2026-09-08.md
...
def stop_tgt_params(item, code):
    ...
    if board in ('cyb', 'chuang', 'kcb') or str(code).startswith(('300', '688')):
        return 3.0, 9.0, board or 'cyb'
    return 2.0, 6.0, board or 'main'
...
for it in items:
    ...
    if day[-1][0] == TODAY:
        if bar:
            day[-1] = bar; replaced += 1   # 用权威收盘棒替换可能的盘中未完成棒
        else:
            already += 1
    elif bar:
        day.append(bar); added += 1
print('当日棒: 新增叠加 %d ｜ 替换 %d ｜ 原已含 %d ｜ 缺 %d' % (added, replaced, already, len(TODAY_BAR) and len([1 for it in items if (it.get('kline',{}).get('day') or [['']])[-1][0] != TODAY and not TODAY_BAR.get(str(it.get('code','')).strip())])))
...
lines.append('> 数据源：腾讯行情（与 westock-mcp 同源公开端点，日K已叠加当日 09-08 收盘棒）；评分引擎：6 维加权模型')
lines.append('')
lines.append('## 一、市场概览')
lines.append('')
...
lines.append('- 本周成交笔数（含今日）：%d 笔（上限 5 笔）。' % week_buys)
lines.append('')
...
lines.append('- 今日 9:30 买入信号因大盘空头（上证<MA20）已暂停交易，本清单为盘后观察候选，非实盘指令。')
lines.append('')
lines.append('---')
...
print('=== SUMMARY ===')
print('total=%d qualified=%d added=%d replaced=%d missing=%d short=%d maxdrop=%.2f week_buys=%d' % (
    len(items), len(qualified), added, replaced, len(missing), len(short_list), max_drop, week_buys))
