...
NEW_SCREEN = (json.load(open(os.path.join(RES, "screen_0826.json"), encoding='utf-8'))
              if os.path.exists(os.path.join(RES, "screen_0826.json")) else {})
# 08-26 新筛候选（westock tool_filter: main_inflow>=1亿 + high_dividend>=4%，已剔除涨停/追高与次新）
...
lines.append('')
lines.append(f'- **数据日**：{DATE}（周三收盘盘后定稿）')
