...
"""
每日收盘选股定稿 (2026-08-31)
- 刷新 import_final.json 中全部候选股的日K线(≈800根, 含当日 08-31)
...
"""
import urllib.request, json, os, datetime, time, math
...
def score_item(item):
    ...
    priceMa = 'above' if closes[-1] >= ma20v else 'below'
    r = rsi(closes)
    a = atr_pct(bars)
    v = vol_class(bars)
    ...
    # 止损适配 10
    if a <= sp * 1.2: s_atr = 10
    elif a <= sp * 1.8: s_atr = 5
    else: s_atr = 0
...
for idx, it in enumerate(items):
    ...
    if not day:
        ...
        if 'kline' not in it or 'day' not in it.get('kline', {}):
...
# 达标筛选
qualified = [it for it in items if it.get('win', 0) >= 70]
...
for it in qualified[:10]:
    board = {'main': '主板', 'cyb': '创业板', 'chuang': '科创板', 'kcb': '科创板'}.get(it['board'], it['board'])
...
# 输出摘要供自动化 memory
print('=== SUMMARY ===')
print('total=%d qualified=%d max_drop=%.2f idx=%s' % (len(items), len(qualified), max_drop, idx_line))
for it in qualified[:3]:
    print('TOP %s %s win=%.1f score=%d' % (it['code'], it['name'].split(' ')[0], it['win'], it['score']))
